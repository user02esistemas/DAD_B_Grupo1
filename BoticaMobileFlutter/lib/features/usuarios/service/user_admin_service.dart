...
class UserAdminService {
  ...
  Future<List<AdminUser>> users() async {
    ...
    if (!result.success || result.data is! List) throw Exception(result.message);
    return (result.data! as List).whereType<Map<String, dynamic>>().map(AdminUser.fromJson).toList();
  }

  Future<List<UserRole>> roles() async {
    final result = await _apiClient.get('/api/usuarios/roles');
    if (!result.success || result.data is! List) throw Exception(result.message);
    return (result.data! as List).whereType<Map<String, dynamic>>().map(UserRole.fromJson).toList();
  }
